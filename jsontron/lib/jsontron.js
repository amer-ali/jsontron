 ...
 *
 */
const parseAssert = function(assert) {
  try {
    const mytest = assert.test;
    ...
    myReport.addError(assert, 'Assert', 'Error in Assert Parsing '+e.message, e.stack);
  }

  return {'id': assert.id, 'result': asrtResult, 'message': assert.message};
};

const evalMessage = function(message) {
  let result;
  try {
    result = eval(message);
  } catch (e) {
    myReport.addError(message, 'Message', 'Error in Message Evaluation ' + e.message, e.stack);
    result = message;
  }
 ...
 *
 */
const validateAssert = function(parsedAssert) {
  try {
    var assrtMsg = parsedAssert.result ? 'successful' : evalMessage(parsedAssert.message);
  } catch (e) {
    myReport.addError(parsedAssert, 'Assert', 'Error in Assert Validation '+e.message, e.stack);
  }
  return assrtMsg;
};
...
const parseRule = function(rule) {
  ...
  try {
    ...
    if (asserts.length < 1) {
      myReport.addWarning(rule, 'Rule', 'Invalid Rule', 'Rule doesn\'t have any assertions defined.');
    }
  } catch (e) {
    myReport.addError(rule, 'Rule', 'Error in Rule Parsing '+e.message, e.stack);
  }
  return asserts;
};
 ...
 *
 */
const validateRule = function(instance, rule) {
  ...
  try {
    ...
    if (ruleParsedContext && Array.isArray(ruleParsedContext) && ruleParsedContext.length == 0) {
      ...
      if (ruleParsedContextNodeSet && Array.isArray(ruleParsedContextNodeSet) && ruleParsedContextNodeSet.length > 0) {
        ruleParsedContextNodeSet.forEach(function(nsElement) {
          ...
          if (Array.isArray(asrts) && asrts.length > 0 && asrts[0] !='') {
            asrts.forEach(function(element) {
              ;
              try {
                const parAssert = parseAssert(element);
                const valAssert = validateAssert(parAssert);
        ...
        myReport.addError(instance, rule, 'Rule Context', 'Error in Rule Validation: Invalid Context Node ', 'The context node set doesn\'t contain valid node - Check Rule Context');
      }
    } else {
...
const parsePattern = function(spattern) {
  let myRuleList = [];
  ...
  try {
    ...
    if (myRuleList.length < 1) {
      myReport.addWarning(spattern, 'Pattern', 'Invalid pattern', 'Pattern doesn\'t have any rules defined.');
    }
  } catch (e) {
    myReport.addError(spattern, 'Pattern', 'Error in Pattern Parsing '+e.message, e.stack);
  }
...
const validatePattern = function(pInstance, pattern) {
  let rls = [];
  try {
...
const parsePatterns = function(psSchema, patternsList) {
  ...
  try {
    ...
    if (!tempPatterns || !Array.isArray(tempPatterns) || tempPatterns.length < 1) { // if no pattern found
      myReport.addError(psSchema, 'Pattern', 'No Pattern found', 'The Schematron rules/schema file should contain atleast 1 pattern');
    ...
    // if patterns list undefine, not an array, empty or contains empty string, all patterns are returned
      ...
      myReport.addWarning(psSchema, 'Pattern', 'Invalid pattern list', 'Since no valid list is provided - All patterns will be processed');
      patterns = tempPatterns;
    } else {
      try {
        const processedPatterns = []; // keep track of valid patterns in the requested list
        ...
        tempPatterns.forEach(function(oElement) {
          patternsList.forEach(function(iElement) {
            try {
              ...
              myReport.addError(psSchema, 'Pattern', 'Parsing Error: '+ e.message, e.stack);
            }
          });
        });
        ...
        try {
          const diffPatterns = patternsList.length - processedPatterns.length;
          if (diffPatterns > 0) { // it means there some invalid patterns are requested
            myReport.addError(psSchema, 'Pattern', 'Invalid Pattern IDs Requested', 'Patterns Requested: '+patternsList + ' Patterns Processed: '+processedPatterns);
          }
        } catch (e) {
          myReport.addError(psSchema, 'Pattern', 'Parsing Error: '+ e.message, e.stack);
        }
...
const validatePatterns = function(psInstance, psSchema, patternsList) {
  let psPatterns = [];
...
const parsePhases = function(phSchema, phaseList) {
  ...
  if (!allPatterns) {
    myReport.addError(phSchema, 'Phase', 'Parsing Error: No Pattern Found', 'Schema should contain atleast one valid pattern');
  ...
  if (!tempPhaseList) {
    try {
      allPatterns.forEach(function(element) {
        ...
        myReport.addWarning(phSchema, 'Phase', 'Parsing Warning. No phase found ', 'This schema doesn\'t have any phase defined . All Patterns will be processed.');
      });
    } catch (e) {
      myReport.addError(phSchema, 'Phase', 'Parsing Error: '+e.message, e.stack);
    }
  } else if (typeof phaseList == 'undefined') {
    // no phase is mentioned while invoking so process all phases
    myPhases = tempPhaseList;
    myReport.addWarning(phSchema, 'Phase', 'Parsing Warning. No phase was provided. ', 'No phase list was provided. All Patterns will be processed.');
  }
  ...
  // else if (typeof phaseList == 'string' && phaseList.toUpperCase() =='DEFAULT'){
    ...
    const dPhase = phSchema.schema.defaultPhase;

    if (!dPhase) { // If the defaultPhase is not defined or is empty, then all phases are active
      myPhases = tempPhaseList;
      myReport.addWarning(phSchema, 'Phase', 'Parsing Error: No Default Phase Found ', 'No Default phase was found in the schema. All phases will be processed');
    } else {
      try {
    ...
    try {
      ...
      try {
        if (phaseList.length > myPhases.length) {
          myReport.addWarning(phSchema, 'Phase', 'Parsing Error: Invalid Phases detected. ', 'Phase (s) Provided: '+ phaseList +' Phases Processed: ' + myPhases );
        }
      } catch (e) {
        myReport.addWarning(phSchema, 'Phase', 'Parsing Error: '+e.message, e.stack );
      }

      // if no valid phase was found
      if (myPhases.length < 1) {
        myPhases = tempPhaseList;
        myReport.addWarning(phSchema, 'Phase', 'Parsing Error: No Valid Phase Specified ', 'Phase list doesnt have any valid phases. All phases will be processed');
      }
    } catch (e) {
      myReport.addError(phSchema, 'Phase', 'Parsing Error: '+e.message, e.stack);
    }
